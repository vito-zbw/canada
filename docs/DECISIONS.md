...
## 2026-05-17 — Remove `wrangler.toml`; rely on Pages dashboard config (reverses earlier entry)

- **Decision:** Delete `wrangler.toml` from the repo. Pure-static Pages projects do not need it. Build config (command = `npm run build`, output directory = `dist`) lives in the Pages dashboard's Build & deployments settings; no environment variables are required at deploy time. The image-fetch policy is unchanged: `scripts/fetch-images.ts` remains a local dev-only tool, the cache under `public/images/` stays committed, and `npm run build` never touches the Pexels API. This entry reverses the earlier 2026-05-17 "Deploy configuration: `wrangler.toml`, no build-time image fetch" decision below.
- **Why:** On first deploy, the presence of `wrangler.toml` made Cloudflare Pages route us into "wrangler-managed deploy" mode and auto-populate the project's Deploy command as `npx wrangler deploy`. That's the Workers subcommand, not Pages — it expects a Worker entry-point and fails with `Missing entry-point to Worker script or to assets directory`, even though `pages_build_output_dir = "./dist"` is set in wrangler.toml. wrangler 4.x's own diagnostic message confirms the path: *"It seems that you have run `wrangler deploy` on a Pages project, `wrangler pages deploy` should be used instead."* The clean fix is to remove `wrangler.toml` entirely: for a pure-static editorial site with no Functions, no Workers, no KV/D1/R2 bindings, the file carries no information the dashboard doesn't already capture, and its presence is what triggered the wrong code path.
- **Alternatives considered:** Keep `wrangler.toml` and override the dashboard Deploy command to `npx wrangler pages deploy dist` (works, but means a hidden dashboard setting silently overrides what the file appears to declare — exactly the kind of split-source-of-truth confusion the file was supposed to eliminate); add a Workers `main = "..."` line to wrangler.toml (would deploy a Worker we don't have); add `[assets] directory = "./dist"` (a Workers config shape that mixes Pages and Workers semantics — confusing and brittle); migrate to a single combined Workers + static deploy (overkill for a site with no server logic).
- **Reversibility:** Easy. Recreating `wrangler.toml` is a few lines, and would only be worth doing once the project gains Pages Functions or any binding worth committing to the repo. If that happens, set the dashboard's Deploy command to `npx wrangler pages deploy dist` *at the same time* — the two must be consistent.
